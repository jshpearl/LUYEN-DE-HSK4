...
# ==============================================================================
# ĐỀ LUYỆN HSK4 - STREAMLIT APP (H41110, H41111, H41218 TRỌN BỘ 100 CÂU MỖI ĐỀ)
# ==============================================================================
...
# --- CSS CAO CẤP: DẠNG PASTEL ĐA SẮC RẠNG RỠ, FORCE LIGHT MODE, CHỮ BÓNG ĐẬM ---
st.markdown("""
<style>
    .stApp {
...
# --- WEBHOOK HOÀN CHỈNH GỬI VỀ TAB HSK4 TRONG GOOGLE SHEETS ---
...
def send_score_to_gsheet(student_name, exam_code, section_name, score_raw, score_100):
    payload = {
        ...
        "score": f"{score_raw} ({score_100:.1f}/100)",
        "sheet_name": "HSK4",
        "tab_name": "HSK4",
        "sheet": "HSK4",
    ...
    try:
        ...
        if res.status_code in [200, 201] or "success" in res.text.lower():
            return True, "Đã gửi kết quả bài làm thành công về tab HSK4 trong Google Sheet!"
        else:
            return True, "Đã ghi nhận kết quả bài làm!"
    except Exception as e:
        return False, f"Lỗi kết nối Webhook: {str(e)}"

# --- TRÌNH PHÁT FILE BÀI NGHE KHỚP MÃ ĐỀ ---
def render_audio_player(exam_code):
    ...
    if os.path.exists(mp3_filename):
        ...
        st.info(f"💡 Tải file nhạc **{mp3_filename}** lên cùng thư mục trên GitHub để phát nhé!")

# --- HÀM HIỂN THỊ HÌNH ẢNH CÂU 96 - 100 THEO MÃ ĐỀ (_1, _2, _3, _4, _5) ---
def render_question_image(exam_code, img_idx, word_hint):
    possible_names = [f"{exam_code}_{img_idx}.jpg", f"{exam_code}_{img_idx}.png", f"{exam_code}_{img_idx}.jpeg", f"images/{exam_code}_{img_idx}.jpg"]
    found_img = None
    ...
    if found_img:
        st.image(found_img, width=280, caption=f"Tranh câu {img_idx+95} (Từ gợi ý: {word_hint})")
    else:
        st.markdown(f"🖼️ *[Khung hình ảnh câu {img_idx+95}: Tải file <code>{exam_code}_{img_idx}.jpg</code> lên GitHub để hiển thị]*", unsafe_allow_html=True)
...
# TẠM THỜI CHỈ LÀM 3 MÃ ĐỀ H41110, H41111, H41218
EXAM_CODES = ["H41110", "H41111", "H41218"]
...
EXAM_DATA = {
    ...
    "p3_read": [
        {
            "num": 66,
            "text": "66. 小时候他们说我更像我爸，尤其是性格，对人热情，说话直接。\n★ 根据这段话，他：",
            "options": [
                "A 很浪漫",
                "B 变帅了",
                "C 特别聪明",
        ...
        {
            "num": 77,
            "text": "77. 他们坚持每天晚上一起散步、聊天儿。感情那么好，让朋友们羡慕。\n★ 朋友们羡慕他们是因为：",
            "options": [
                "A 很诚实",
                "B 要去留学",
                "C 感情很好",
        ...
        {
            "num": 78,
            "text": "78. 《现代汉语词典》是一本很有用的工具书，遇到不会读的字可以帮你。\n★ 《现代汉语词典》可以帮助我们：",
            "options": [
                "A 认识汉字",
                "B 熟悉环境",
                "C 适应社会",
        ...
        {
            "num": 79,
            "text": "79. 每个人都会做梦，说自己没做过梦的人只不过是忘记了。\n★ 这段话告诉我们：",
            "options": [
                "A 做梦的原因",
                "B 所有人都会做梦",
                "C 梦可以解释",
        ...
        {
            "num": 82,
            "text": "82. 明白自己想做什么很重要，知道怎么去做更为关键。\n★ 这段话主要谈什么？",
            "options": [
                "A 理想",
                "B 专业",
                "C 收入",
        ...
        {
            "num": 84,
            "text": "84. 老人喜欢热闹，害怕孤单，他们需要别人的重视和关心。\n★ 老人更喜欢：",
            "options": [
                "A 开玩笑",
                "B 住在农村",
                "C 获得重视",
        ...
        {
            "num": 85,
            "text": "85. 多鼓励老人参加一些社会活动，让他们觉得有“被需要”的感觉。\n★ 这段话主要谈什么？",
            "options": [
                "A 要有信心",
                "B 要积累经验",
                "C 遇事要冷静",
    ...
    with exam_tabs[idx]:
        ...
        with sub_tab_listen:
            ...
            if st.button(f"🚀 NỘP BÀI PHẦN NGHE MÃ {exam_code}", key=f"btn_sub_l_{exam_code}"):
                if not student_name.strip():
                    ...
                    c1 = sum(1 for q in data["p1_listen"] if u_p1.get(q['num']) == q['correct'])
                    c2 = sum(1 for q in data["p2_listen"] if u_p2.get(q['num']) == q['correct'])
                    c3 = sum(1 for q in data["p3_listen"] if u_p3.get(q['num']) == q['correct'])
                    tot_c = c1 + c2 + c3
                    ...
                    for q in data["p1_listen"]:
                        if u_p1.get(q['num']) != q['correct']:
                            ...
                            with st.expander(f"📖 查看听力文本 (Script Câu {q['num']})"):
                                st.write(q['script'])
                    ...
                    for q in data["p3_listen"]:
                        if u_p3.get(q['num']) != q['correct']:
                            st.markdown(f"❌ **Câu {q['num']}**: Bạn chọn `{u_p3.get(q['num'])}` | Đáp án đúng: **{q['correct']}**")
                            with st.expander(f"📖 查看听力文本 (Script Câu {q['num']})"):
                                st.write(q['script'])
        ...
        with sub_tab_write:
            ...
            for i, q in enumerate(data["p2_write"]):
                ...
                render_question_image(exam_code, q['img_num'], q['word'])
                ans = st.text_area(f"Nhập câu đặt theo tranh cho câu {q['num']}:", key=f"w_w2_{exam_code}_{q['num']}")
                u_w2[q['num']] = ans
                st.markdown("</div>", unsafe_allow_html=True)
                
            st.info("📌 Note: Các câu từ 96-100 (đặt câu theo tranh) cô Ngọc sẽ chấm cụ thể sau. Điểm hiển thị bên dưới chỉ mang tính chất tương对.")
            ...
            if st.button(f"🚀 NỘP BÀI PHẦN VIẾT MÃ {exam_code}", key=f"btn_sub_w_{exam_code}"):
                if not student_name.strip():
                    ...
                    for q in data["p1_write"]:
                        if u_w1.get(q['num']) not in q['correct']:
                            st.markdown(f"❌ **Câu {q['num']}**: Bạn viết `{u_w1.get(q['num'])}` | Đáp án đúng: **{q['correct'][0]}**")
